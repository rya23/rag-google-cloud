...
const INGEST_BASE = (window.APP_CONFIG && window.APP_CONFIG.ingestBase) || "http://localhost:8030";
...
async function refreshJobs() {
  const response = await fetch(`${INGEST_BASE}/ingest/jobs`);
  if (!response.ok) {
...
uploadForm.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch(`${INGEST_BASE}/ingest/file`, {
      method: "POST",
      body: formData,
    });
